...
class BryantPipeline:

    # Database setup
    conn = sqlite3.connect('bryant.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bryant`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bryant WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bryant (date, title, link, content)"
